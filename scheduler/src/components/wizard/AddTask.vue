            ...
            <div class="w-full grid grid-rows-2">
				...
				<div class="button-group-row w-full justify-between row-start-2">
					<div class="button-group-row mt-2">
                        <button @click="showWizard = false" :id="getIdKey('close-add-task-btn')" name="close-add-task-btn" class="mt-1 btn btn-danger">Close</button>
					</div>
					<div class="button-group-row mt-2">
                        <!-- <button v-if="!adding" id="add-task-btn" class="btn btn-primary object-right justify-end mr-4 h-fit w-full" @click="addTaskBtn">Add Task</button>
...
const taskTemplates = inject<Ref<TaskTemplate[]>>('task-templates')!;
