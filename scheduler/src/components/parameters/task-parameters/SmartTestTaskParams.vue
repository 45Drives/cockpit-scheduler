...
- Device Alias: A user-friendly alias for the device which corrosponds to the slot number the drive is plugged into.
  (set by 45Drives Tools - dalias)" 
                ...
                <select :id="'disk-identifier'" v-model="selectedIdentifier" name="disk-identifier" class="text-default bg-default mt-1 block w-full input-textlike sm:text-sm sm:leading-6">
            ...
            <div name="disk-selection">
                ...
                <ul :id="'disk-selection'" role="list" class="grid grid-cols-6 grid-rows-2 gap-2"  :class="[diskSelectionError ? 'outline outline-1 outline-rose-500 dark:outline-rose-700 rounded-md' : '']">
                    <li v-for="(disk, diskIdx) in diskList" :key="diskIdx" class="my-1">
                        <button class="flex min-w-fit w-full h-full border border-default rounded-lg py-1 px-2"
                            ...
                            <label :for="`disk-${diskIdx}`" class="flex flex-col w-full text-sm gap-0.5">
                                <input :id="`disk-${diskIdx}`" v-model="selectedDisks" type="checkbox" :value="`${getDiskIDName(diskList, selectedIdentifier, disk.name).diskPath}`" :name="`disk-${disk.name}`"
                                ...
                                <p class="truncate text-sm font-medium text-default" :title="getDiskIDName(diskList, selectedIdentifier, disk.name).diskName">{{ truncateName(getDiskIDName(diskList, selectedIdentifier, disk.name).diskName, 8) }}</p>
        ...
        <!-- BOTTOM -->
        <div name="test-options" class="border border-default rounded-md p-2 col-span-2 row-span-1 row-start-2 bg-accent">
            ...
            <div class="col-span-1">
                <label :for="'test-type'" class="mt-1 block text-sm leading-6 text-default">
                    Test Type
                    <InfoTile class="ml-1" title="Select the type of S.M.A.R.T. test to run on the disk.
...
- Conveyance Test: Specific to ATA drives, this test checks for any damage that may have occurred during transport. It typically completes in a few minutes." />
                ...
                <select :id="'test-type'" v-model="selectedTestType" name="test-type" class="text-default bg-default mt-1 block w-full input-textlike sm:text-sm sm:leading-6"
