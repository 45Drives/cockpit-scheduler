<template>
    <Modal @close="closeModal" :isOpen="showEditTaskWizard" :margin-top="'mt-10'" :width="'w-3/5'"
        ...
        <template v-slot:title>
            Edit <span class="text-base">{{taskInstance.name}}</span> <br /><span
                class="text-base text-muted italic">{{taskInstance.template.name}}</span>
        </template>
        <template v-slot:content>
            <div>
                <ParameterInput ref="parameterInputComponent" :selectedTemplate="taskInstance.template"
                    :task="taskInstance" />
            </div>
        </template>
            ...
            <div class="w-full">
                <div class="button-group-row w-full justify-between">
                    <div class="button-group-row">
                        <button @click.stop="closeBtn()" id="close-edit-task-btn" name="close-edit-task-btn"
                            class="btn btn-danger h-fit w-full">Close</button>
                    </div>
                    <div class="button-group-row">
                        <button v-if="saving" disabled id="editing-task-btn" type="button"
                        ...
                        <button v-else id="edit-task-btn" class="btn btn-primary h-fit w-full"
                            @click="saveChangesBtn">Save Task</button>
                    </div>
                </div>
            </div>
        </template>
    ...
    <div v-if="showSaveConfirmation">
        <component :is="confirmationComponent" @close="updateShowSaveConfirmation" :showFlag="showSaveConfirmation"
            :title="'Save Task'" :message="'Save your edits?'" :confirmYes="confirmSaveChanges" :confirmNo="cancelEdit"
            :operation="'saving'" :operating="saving" />
    </div>

    <div v-if="showCloseConfirmation">
        <component :is="closeConfirmationComponent" @close="updateShowCloseConfirmation"
            :showFlag="showCloseConfirmation" :title="'Cancel Edit Task'"
            :message="'Are you sure? Any changes will be lost.'" :confirmYes="confirmCancel" :confirmNo="cancelCancel"
            :operation="'canceling'" :operating="cancelingEditTask" />
    </div>
